...
| StabilityDictionaryE2eTest：every registered fact type lands on disk in a real ide and files are preserved | 177.5s | 31 个登记 name 全部经真实采集管线落盘于**唯一** jsonl（critical 与 diagnostic 行同文件）；文件名匹配 `^sc-[0-9a-f]{12}-pr-[0-9a-f]{12}\.jsonl$` 且全程稳定；7 kind 齐备、metrics/logs/dual 投影齐备、error 计数/详情分道；恰 1 started、1 shutdown |
